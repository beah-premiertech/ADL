...
public static class AdManager
{
    ...
    public static async Task GetAllDataAsync(string UserDomain, string Password)
    {
        ...
        GetDomains(UserDomain, Password);
        await GetAllOus(UserDomain, Password);
        new Thread(() => { GetAllOus(); }).Start();

        var deviceTask = GetAllDevicesAsync(UserDomain, Password);
        var userTask = GetAllUsersAsync(UserDomain, Password);
        var groupTask = GetAllGroupsAsync(UserDomain, Password);
        await Task.WhenAll(deviceTask, userTask, groupTask);
    }
    private static void GetDomains(string UserDomain, string Password)
    {
    ...
    private static async Task GetAllOus(string UserDomain, string Password)
    {
        AdRawOus.Clear();
        AdOus.Clear();
        AdOU = new AdOu { Name = "All", Domain = "All", FullPath = "All", Path = "All" };
        ...
        try
        {
            using (PowerShell ps = PowerShell.Create())
            {
                ...
                foreach (var Domain in Domains)
                {
                    AdOU.Children.Add(new AdOu { Name = Domain, Domain = Domain, Path = Domain, FullPath = $"DC={Domain.Replace(".",",DC=")}" });
                    ps.Commands.Clear();
